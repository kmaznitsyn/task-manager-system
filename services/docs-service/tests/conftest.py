"""Test fixtures for docs-service — mirrors task-service patterns."""
...
def stub_publisher(monkeypatch, request):
    """Replace publish_document_event with a no-op spy on every test."""
    calls: list = []
    ...
    if request.node.get_closest_marker("no_stub_publisher") is None:
        monkeypatch.setattr(publisher_module, "publish_document_event", _fake_publish)
    return calls
